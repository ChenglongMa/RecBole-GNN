...
class PGE(nn.Module):
    ...
    def __init__(self, config, nfeat, nnodes, nhid=128, nlayers=3):
        super().__init__()
        # if config.dataset in ['ogbn-arxiv', 'arxiv', 'flickr']:
        #    nhid = 256
        # if config.dataset in ['reddit']:
        #    nhid = 256
        #    if config.reduction_rate==0.01:
        #        nhid = 128
        #    nlayers = 3
        #   # nhid = 128
    ...
    def forward(self, x: torch.Tensor, inference=False):
        ...
        if self.args.dataset == "reddit" and self.args.reduction_rate >= 0.01:
            ...
            edge_embed = []
            for idx in splits:
                tmp_edge_embed = torch.cat(
                    [x[edge_index[0][idx]], x[edge_index[1][idx]]], axis=1
                )
