...
class OurGNN(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        for i in range(self.hop):
            global_agg_list.append(
                GlobalAggregator(self.embedding_size, self.dropout_gcn)
            )
    ...
    def construct_global_graph(self, dataset):
        ...
        for i in tqdm(range(self.n_items), desc="Sorting: "):
            sorted_out_edges = [
                v
                for v in sorted(
                    global_graph[i].items(), reverse=True, key=lambda x: x[1]
                )
            ]
        ...
        return torch.LongTensor(global_adj).to(self.device), torch.FloatTensor(
            global_weight
        ).to(self.device)
    ...
    def forward(self, x, edge_index, edge_attr, alias_inputs, item_seq_len):
        ...
        item_neighbors = [
            F.pad(
                x[alias_inputs],
                (0, self.max_seq_length - x[alias_inputs].shape[1]),
                "constant",
                0,
            )
        ]
        ...
        for i in range(self.hop):
            item_sample_i, weight_sample_i = (
                self.global_adj[item_neighbors[-1].view(-1)],
                self.global_weight[item_neighbors[-1].view(-1)],
            )
        ...
        h_global = entity_vectors[0].view(
            batch_size, self.max_seq_length, self.embedding_size
        )
